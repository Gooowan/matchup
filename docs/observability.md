...
```bash
# On the server (repo already cloned, .env configured):
cd ~/matchup
git pull
chmod +x scripts/observability-up.sh scripts/benchmark.sh

# One-shot: generates METRICS_TOKEN + secrets/metrics_token, starts stack
./scripts/observability-up.sh --prod

# Or full install (systemd + optional Grafana HTTPS):
sudo REPO_DIR=~/matchup ./scripts/setup-observability.sh
sudo systemctl start matchup-observability
```

**Server `.env` checklist** (edit manually if not already set):

```bash
METRICS_TOKEN=<auto-generated by observability-up.sh if empty>
OTEL_ENDPOINT=http://tempo:4318
GRAFANA_PASSWORD=<strong password>
```

**Baseline load test before seed:**

```bash
./scripts/benchmark.sh your@email.com yourpassword https://matchup-api.example.com 5m
# Open Grafana → MatchUp → API Overview during the run
# Then: seed-all → repeat benchmark with same args
```

See `scripts/observability-up.sh` and `scripts/benchmark.sh` for details.

### Production Server Setup (legacy script details)
