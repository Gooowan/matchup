<script lang="ts">
	...
	onMount(async () => {
		// Dynamically import Leaflet (only in browser / Capacitor)
		const leaflet = await import('leaflet');
		L = leaflet.default || leaflet;

		// Fix default icon paths for bundled apps
		...
		// CartoDB Positron — clean light tiles matching the design
	...
	$effect(() => {
		...
		if (map && L) {
			activeCategory; // track dependency
			addMarkers();
		}
	});
</script>
...
<!-- Screen wrapper -->
<div class="relative h-[100dvh] overflow-hidden" style="background: #dae1eb;">
	...
	<div class="pointer-events-none absolute inset-0" style="z-index: 10;">
		<!-- Search bar -->
		<div
			class="pointer-events-auto absolute left-4 right-4 flex items-center gap-3 bg-white"
			style="
			...
			"
		>
			<i class="fi fi-rr-search" style="font-size: 20px; line-height: 1; color: #171717; flex-shrink: 0;"></i>
			<span class="text-[14px] font-semibold" style="color: #171717;">Search</span>
		</div>

		<!-- Category tab bar -->
		<div
			class="pointer-events-auto absolute left-4 right-4 flex items-center gap-1 bg-white"
			style="
		...
		>
			{#each (['schools', 'dancers', 'tailoring', 'events'] as const) as cat}
				<button
					...
					style="
						height: 28px;
						background: {activeCategory === cat ? '#696969' : 'transparent'};
						color: {activeCategory === cat ? 'white' : '#171717'};
					"
	...
	{#if selectedEntity}
		{#if selectedEntity.type === 'events'}
			...
			{#if selectedEntity.date}
				<p class="mb-1 text-[14px] font-extrabold" style="color: #171717;">{selectedEntity.date}</p>
			{/if}
			<p class="mb-2 text-[18px] font-black" style="color: #171717;">{selectedEntity.name}</p>
			<div class="mb-1 flex items-center gap-1.5">
				<i class="fi fi-rr-marker" style="font-size: 15px; color: #171717;"></i>
				<span class="text-[12px] font-medium" style="color: #171717;">{selectedEntity.location}</span>
			</div>
			{#if selectedEntity.ticketsUrl}
				<div class="mb-1 flex items-center gap-1.5">
					<i class="fi fi-rr-ticket" style="font-size: 15px; color: #171717;"></i>
					<span class="text-[12px] font-medium" style="color: #171717;">{selectedEntity.ticketsUrl}</span>
				</div>
			{/if}
			{#if selectedEntity.description}
				<p class="mb-4 text-[12px] font-medium leading-snug" style="color: #171717;">{selectedEntity.description}</p>
			{/if}
		...
		{:else}
			...
			<div class="mb-4 flex items-start gap-4">
				...
				<div class="flex flex-col gap-1">
					<p class="text-[18px] font-black" style="color: #171717;">{selectedEntity.name}</p>
					...
					{#if selectedEntity.ratingCount}
						<div class="flex items-center gap-1.5">
							<span class="text-[12px] font-medium" style="color: #171717;">★★★★★ ({selectedEntity.ratingCount})</span>
						</div>
					{/if}
				</div>
			</div>
			...
			{#if selectedEntity.hours}
				<p class="mb-1 text-[12px] font-medium" style="color: #171717;">Open hours: {selectedEntity.hours}</p>
			{/if}
			{#if selectedEntity.address}
				<p class="mb-3 text-[12px] font-medium" style="color: #171717;">Address: {selectedEntity.address}</p>
			{/if}
			...
			<div class="flex items-center gap-2">
				...
				<button class="flex h-[38px] items-center gap-4 rounded-[65px]" style="background: #696969; padding: 0 24px 0 16px;">
