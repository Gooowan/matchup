...
function createDesimBalanceStore() {
	...
	return {
		...
		async fetchBalance() {
			...
			const resp = await authFetch('/desim/balance');
			if (!resp.ok) {
		...
		async fetchTransactions(page: number = 1, take: number = 10) {
			...
			const resp = await authFetch(`/desim/transactions?page=${page}&take=${take}`);
			if (!resp.ok) {
...
export const desimBalanceStore = createDesimBalanceStore();
